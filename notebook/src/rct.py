...
A collection of methods to be used in signal analysis
...
# Returns the IQ data from the SDR binary file
...
def plotPeriods(t, periods, color='y', linestyle='--'):
# Plots predicted period divisions
    for p in periods:
        plt.axvline(t[p],c=color,linestyle=linestyle)

        
def plotPulses(t, pulses, color='m'):
# Plots predicted pulse locations
    for p in pulses:
...
# Returns the index of period divisions centered about the pulse

# Assuming that the provided max index is a pulse, finds the temporal 
#   location of each pulse period border centered about the pulse. 
#   Then fits the time to the nearest index
...
# Returns the index of expected pulse locations
...
def fftFromFiles(raw_files, fft_size, target_bin, fs, w='boxcar',ovlap=None):
# Returns the amplitude of the target frequency bin across time
#    Adds to the original function by using welch's method allowing windowing and overlap
#    If the sample is not centered on the pulse use a boxcar window
...
# Returns a more precise representation of the pulse period
...
# Returns a more precise representation of the pulse width
    m_idx = np.argmax(data)
...
# Returns a dictionary of variables extracted from the metafiles
    variables = []; values = [];
...
def calcNoiseFloor(Px,peaks,Fx,pW):
# Returns the noise floor power by averaging power between detected peaks
    noise = []
    for i in range(0,len(peaks)-1):
        a,b = peaks[i]+int(np.ceil(pW*Fx)), peaks[i+1]-int(np.ceil(pW*Fx))
        noise.extend(Px[a:b])
    return np.mean(noise)


def calcSNR(s_n,n):
# Returns the SNR of a noisy signal at a point
    return 10 * np.log10((s_n-n)/n)


def getSNR(Px,peaks,Fx,pW):
# Returns a list of SNR values for each peak
    nf  = calcNoiseFloor(Px,peaks,Fx,pW)
    snr = []
    for p in peaks:
        snr.append(calcSNR(Px[p],nf))
    return snr
...
##%% The following functions/classes are deprecated and should not be used.
## They are included for posterity as their implementations differ from the updated methods,
##  but serve as examples for expanding on the current methods.
#
## DEPRECATED
