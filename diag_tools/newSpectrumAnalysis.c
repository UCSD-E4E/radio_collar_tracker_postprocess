...
void init_buffers() {
	...
	col_avg = (double*) calloc(num_col, sizeof(double));
}
	...
	for (int i = 0; i < num_files; i++) {
		...
		for (int j = 0; j < num_fra_p_file; j++) {
			...
			for (int k = 0; k < frame_size; k++) {
				in_phase[k]     = ((double)frame[j][2 * k] - 127) / 128;
				quadrature[k]   = ((double)frame[j][2 * k + 1] - 127) / 128;
				signal_cpx[k]   = in_phase[k] + I * quadrature[k];
				cpxAux += signal_cpx[k];
			}
			...
			while (k < sld_fft_stps) {
				...
				for (int l = 0; l < num_col; l++) {
					...
					// FIXME: Set sane noise average!
					col_avg[l] += pow(fft_abs[fft_bin[l]], 2);
					col_avg[l] /= 2.0;
				}
			...
			for (int l = 0; l < num_col; l++) {
				...
				// SNR caculation
				pulse_snr[l][cur_fr] = 10 * log10( signal_pwr / col_avg[l]);
